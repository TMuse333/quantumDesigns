...
const Infographic = () => {
    ...
    const oscillatingAnimation = {
        scale: [1, 1.2, 1], // Oscillate between scale 1 and scale 1.2
        transition: {
          duration: 0.5,
      ...
      const contentStyle = (index) => {
        return {
          ...
          overflow: 'scroll',
          transform: isHovered === index && !expandedIndices.includes(index) ? 'scale(1.2)' : 'scale(1)',
          ...(isHovered === index && !expandedIndices.includes(index) ? oscillatingAnimation : {}), // Apply the oscillating animation when hovered
        };
      };
