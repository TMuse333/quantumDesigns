...
const Process = () => {
    ...
    return (
     ...
     <IoIosArrowBack onClick={prevTenet} className="arrow" />
            ...
            <AnimatePresence >
  <motion.div key={currentTenet} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <h2 className="tenet-title">{tenets[currentTenet].name}</h2>
    <motion.p  key={currentTenet}
    className="tenet-desc" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
