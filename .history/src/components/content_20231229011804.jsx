...
const Content = ({ img, title, description, rev, link, linkText, descriptionIntro, boldTitle, boldDescription }) => {
  ...
  const makeBold = (text, boldWords, maxOccurrences = 1) => {
    ...
    return words.map((word, index) => {
      ...
      if (boldWords.includes(word) && wordOccurrences[word] <= maxOccurrences) {
        return <span className="bold-700" key={index}>{text} </span>;
      } else {
        return `${word} `;
      }
    });
      ...
      <div className={`image-text-box ${rev ? 'reverse' : ''}`}>
        ...
        {rev ? (
          ...
          <div className="description-box">
            ...
            {title && <h2 className="title-text">{makeBold(title, boldTitle)}</h2>}
            {/* Render the description with specified words bold and link/button if provided */}
            <p className='description-text'>
              {makeBold(description, boldDescription)}
              {link != null &&
                <Link to={link}>
        ...
        {rev ? null : (
          ...
          <div className="description-box">
            ...
            <h2 className="title-text">
              {makeBold(descriptionIntro, boldTitle)}
            </h2>
            {/* Render the description with specified words bold and link/button if provided */}
            <p className='description-text'>
              {makeBold(description, boldDescription)}
