...
const Example = ({ image, id, imageId }) => {
  ...
  const width = isInView ? '90vw' : '48vw';
  const height = isInView ? '60vw' : '50vw';
  ...
  return (
    <motion.div
      ...
      initial='initial'
      animate={isInView ? 'target' : 'initial'} // Start animation only when in view
      transition={{
        ease: 'easeInOut',
        duration: 1,
      }}
    >
      <motion.img
        ...
        src={image}
        alt='Your Image'
        style={{ opacity: 0, width: '100%', height: '100%' }} // Ensure image takes full space within its container
        variants={{
          animate: {
            opacity: 1,
          }
