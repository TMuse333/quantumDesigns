...
const Content = ({ img, title, description, rev,
  ...
  useEffect(() => {
    setAnimationComplete(false); // Reset the animation complete flag when isAnimated changes
  }, [isAnimated]);
  ...
  const handleAnimationComplete = () => {
    ...
    if (!animationComplete) {
      const boldedDescription = makeBold2(description, ["bold", "words"]); // Replace with your bold words
  ...
  const visibleWidth = isAnimated ? '100%' : '0%';
